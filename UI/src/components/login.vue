    ...
    <div class="login-box">
      ...
      <a-form-model :model="loginForm" ref="loginFormRef" :rules="loginFormRules" label-width="0px" class="login-form"
        ...
        <a-form-model-item prop="username" label="用户名">
          <a-input v-model=loginForm.username prefix-icon="el-icon-user"></a-input>
        </a-form-model-item>


        <!--          密码-->
        <a-form-model-item prop="password" label="密码">
          <a-input type="password" v-on:keyup.enter.native="login" v-model=loginForm.password
                   prefix-icon="el-icon-lock"></a-input>
        </a-form-model-item>
        <a-form-model-item class="btns" :wrapper-col="{ span: 14, offset: 4 }">
          <a-button type="primary" @click="login()">登录</a-button>
          <a-button type="info" style="margin-left: 10px" @click="resetLoginForm()">重置</a-button>
        </a-form-model-item>
      </a-form-model>
    ...
    login() {
      ...
      this.$refs.loginFormRef.validate(valid => {
        ...
        this.$http.post('login', {username: this.loginForm.username, password: pass}).then(function (result) {
          // console.log(result);
          // that.$message.success(result.toString());
...
.login-container {
  ...
  //height: 100%;
  background: linear-gradient(217deg, rgb(0, 247, 255), rgba(255, 0, 0, 0) 70.71%),
  linear-gradient(127deg, rgb(217, 0, 255), rgba(0, 255, 0, 0) 70.71%),
  linear-gradient(336deg, rgb(76, 255, 0), rgba(0, 0, 255, 0) 70.71%);
  height: 100%;
}
